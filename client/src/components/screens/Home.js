                ...
                data.map(item=>{
                    return(
                        <div className="card home-card" key={item._id}>
                            ...
                            :
                            <i className="material-icons" style={{color:"black"}} onClick={()=>{likePost(item._id)}} >favorite_border</i>
                            }
                            {item.likes.includes(state._id)
                            ...
                            <h6>{item.likes.length} likes</h6>
                                ...
                                <p>{item.body}</p>
                                {
                                    item.comments.map(record=>{
                                        return(
                                        <h6 key={record._id}><span style={{fontWeight:"500",marginRight:10}}>{record.postedBy.name}</span>{record.text}</h6>
                                        )
                                    })
                                }
                                <form onSubmit={(e)=>{
